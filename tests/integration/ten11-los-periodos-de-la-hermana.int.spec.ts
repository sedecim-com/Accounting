...
let b: Fixture;
let srv: Servidor;
const CALENDARIO_A = randomUUID();
const CALENDARIO_B = randomUUID();

beforeAll(async () => {
  a = await crearInquilino('TEN-11 periodos A');
  b = await crearEntidadHermana(a, 'TEN-11 periodos B');
  for (const [id, fx] of [[CALENDARIO_A, a], [CALENDARIO_B, b]] as const) {
    await query(
...
const periodosDe = async (calendario: string): Promise<string[]> => {
...
describe('los periodos del calendario de la hermana', () => {
  it('contesta 404, y no escribe un solo periodo', async () => {
    const r = await pedir(srv, 'POST', `/v1/payroll/pay-schedules/${CALENDARIO_B}/generate-periods`, { count: 3 });
    expect(r.status, `contestó ${r.status}`).toBe(404);
    expect(await periodosDe(CALENDARIO_B)).toEqual([]);
  });

  it('el 404 es idéntico al de un calendario que no existe', async () => {
    const ajena = await pedir(srv, 'POST', `/v1/payroll/pay-schedules/${CALENDARIO_B}/generate-periods`, { count: 3 });
    ...
    expect(normalizar(ajena.body, CALENDARIO_B)).toBe(normalizar(fantasma.body, fantasmaId));
  });

  it('EL CONTRAPESO: el calendario propio SÍ genera, y con las fechas correctas', async () => {
    const r = await pedir(srv, 'POST', `/v1/payroll/pay-schedules/${CALENDARIO_A}/generate-periods`, { count: 3 });
    ...
    expect(await periodosDe(CALENDARIO_A)).toEqual(['2026-01-01', '2026-01-16', '2026-02-01']);
  });
  ...
  it('y la segunda llamada continúa donde terminó la primera', async () => {
    ...
    const r = await pedir(srv, 'POST', `/v1/payroll/pay-schedules/${CALENDARIO_A}/generate-periods`, { count: 1 });
    expect(r.status, JSON.stringify(r.body)).toBe(200);
    expect(await periodosDe(CALENDARIO_A)).toEqual(['2026-01-01', '2026-01-16', '2026-02-01', '2026-02-16']);
  });

  it('un count fuera de rango se rechaza antes de escribir', async () => {
    const r = await pedir(srv, 'POST', `/v1/payroll/pay-schedules/${CALENDARIO_A}/generate-periods`, { count: 100000 });
