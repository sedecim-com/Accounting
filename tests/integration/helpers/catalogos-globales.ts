 ...
 *
 * `f07a` ya había tropezado con esto y se defendió a mano, contando las filas
 * del catálogo al llegar para devolverlo igual en su `afterAll`. Esa defensa
 * tenía dos grietas: hay que acordarse de escribirla, y un conteo no ve un
 * UPDATE que deja el mismo número de filas. Este archivo la sustituye por algo
 * que NO hay que recordar —`apartarCatalogos`, una línea— y por un vigilante
 * (`vigilante-catalogos.ts`) que toma la huella antes y después de CADA
 * archivo: el que ensucie un catálogo falla ÉL MISMO, con el nombre de la
...
export function explicarDesajustes(archivo: string, desajustes: Desajuste[]): string {
  ...
  return [
    ...
    'tu beforeAll — y no un conteo a mano, que no ve un UPDATE que no cambia el',
    'número de filas:',
    '',
    "    apartarCatalogos('<la tabla>');",
    '',
    'de tests/integration/helpers/catalogos-globales.ts.',
    '',
