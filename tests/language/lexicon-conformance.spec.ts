...
const EDGE_CASES: readonly EdgeCase[] = [
  ...
  { input: '0', tokens: ['0'], klass: 'neutral', why: 'un dígito suelto' },
  {
    ...
    tokens: ['2', 'fa'],
    klass: 'neutral',
    why: 'EL INCÓMODO SE RESOLVIÓ (#197): el corte por dígitos lo deja en dos trozos cortos y sale neutro, que es lo que manda la regla de la duda; antes caía en inglés por defecto',
  },
  {
    input: 'tasa8',
    tokens: ['tasa8'],
  ...
  },
  {
    ...
    tokens: ['tasa', '9'],
    klass: 'es',
    why: 'EL INCÓMODO SE RESOLVIÓ (#197): sin curar y sin embargo español, porque el corte por dígitos alcanza justo lo que el léxico NO conoce; `tasa8` de arriba sigue entero por lo contrario, y ese par es la prueba de que el corte es selectivo',
  },
  ...
  },
  {
    ...
    tokens: ['ano'],
    klass: 'es',
    why: 'EL INCÓMODO SE RESOLVIÓ (#197): la eñe ya no es separador y el token plegado casa con la raíz `ano`, que esta misma fusión tuvo que añadir al JSON — sin esa palabra el plegado dejaba `ano` fuera de toda lista y el español salía INGLÉS, peor que antes',
  },
  {
    input: 'añoFiscal',
    tokens: ['ano', 'fiscal'],
    klass: 'es',
    why: 'EL INCÓMODO SE RESOLVIÓ (#197): el nombre español entero ya no es invisible para la puerta; `fiscal` es neutro y `ano` decide',
  },
  {
    input: 'ñ',
    tokens: ['n'],
    klass: 'neutral',
    why: 'un carácter no ASCII SÍ deja token desde #197 —antes era separador y no dejaba ninguno—; la clase no cambia, porque una letra sola sigue sin decir idioma',
  },
  {
    input: '#saldo',
    tokens: ['saldo'],
  ...
  { input: 'saldo inicial', tokens: ['saldo', 'inicial'], klass: 'es', why: 'el espacio ya venía contemplado' },
  {
    ...
    tokens: ['v', '2'],
    klass: 'neutral',
    why: 'dos caracteres con dígito: el corte de #197 lo parte, y la clase no se mueve porque los dos trozos son neutros',
  },
