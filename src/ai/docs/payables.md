...
- Foreign-currency bill payment (R4): the liability is settled at the DOCUMENT's rate (what it was born worth); the cash leaves at the PAYMENT's rate — that rate resolves from the `fuente_tipo_cambio` policy (DOF by default) and FAILS CLOSED if the chosen source has no rate for the payment date, even if another source does. The realized difference lands in 4320 (Utilidad Cambiaria) or 6320 (Pérdida Cambiaria), pro-rata on partial payments; the payment log records the difference and the rate applied. If a user asks why a payment stopped for a missing DOF rate, the answer is `fx rate set` with the published value — the engine never borrows the FIX for a DOF firm.
...
- Approving is scoped to the ACTIVE entity, on every surface. A bill that belongs to a sibling company of the same firm answers 404 — the very same 404 as a bill id that does not exist, on purpose, so the answer never confirms that another company's bill is there. Until TEN-11 (#235) the REST route did not scope at all: approving the sibling's bill posted an entry into ITS ledger and consumed its folio. If a human says a bill "is not found" but they can see it, the first question is which entity their session is in.
