...
- `bank statement import <file...>` · `estado-cuenta importar` is IA ✓ **because it cannot reach the ledger**: it parses a file into the bank staging tables and posts nothing. Formats: csv (with per-bank profiles), camt053, mt940. Dedupe is by FILE: the whole file by sha256, so re-importing the same file is refused, not silently duplicated. Each line also carries a content hash the DATABASE computes and imposes — you cannot forge it — but that hash is a FINGERPRINT, not a key: two legitimately identical movements on the same day (two 50.00 fees) share it and BOTH belong in the books. Never tell a user a repeated line was dropped as a duplicate.
