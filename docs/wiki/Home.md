...
El repositorio se llama `Accounting` y el paquete `accounting-core` por su origen: un servidor REST (y, hasta T14b, también GraphQL). Ese motor sigue vivo y es el que el agente opera, pero el producto es el CLI.
...
- **GraphQL se retiró** ([#101](https://github.com/sedecim-com/Accounting/issues/101)). Era una segunda puerta al mismo motor —quince mutaciones declaradas, catorce implementadas— montada fuera del prefijo auditado `/v1`, así que lo que pasaba por ella no dejaba la fila de PETICIÓN en `audit_log`. Estaba apagada tras una bandera y no la consumía nadie: ningún cliente en el árbol, ningún `.graphql`, y `npm run graphql:codegen` sin binario. Una superficie que nadie ejerce no se blinda: se quita. Y el criterio que la vigilaba se ponía verde con sólo mudar su montaje de archivo, así que el que lo sustituye mide un hecho positivo — que el paquete no esté en las dependencias.
