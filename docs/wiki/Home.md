...
- **GraphQL está desmontado por omisión.** Vive fuera del prefijo auditado `/v1` y sus mutaciones no comprueban permisos: cualquier principal autenticado podría postear al mayor sin dejar fila de auditoría. Se apagó tras una bandera (`GRAPHQL_ENABLED=true`) en vez de borrarse, y no vuelve hasta que las mutaciones tengan control de permisos y el montaje se mueva dentro del prefijo auditado.
