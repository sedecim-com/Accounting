...
2. **I6–I11, the user reads in their language**: a locale resolver (`--locale`, `es-MX` by default), a **typed** catalog (a key without a Spanish translation is a compile error) with an own formatter over `Intl`; the CLI kernel and the `bank` pilot; the leaves by family; the API with `Accept-Language` and frozen codes; the panel asking in the accountant's language without touching what is persisted; reports, `doctor` and the agent.
3. **I12–I15, the renames**: the *codemod* that updates criteria, thresholds, manifest and catalog in the same commit, rehearsed on a module without coupling; `scripts/` with permanent `npm` aliases; the internal API that crosses folders; SAT, DIOT, Anexo 24 and payroll without touching a byte of what goes to the SAT.
