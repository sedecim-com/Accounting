...
**vivo · US (la tabla admite `pension_alimenticia` MX; el motor no corre para MX).** Se invoca desde el orquestador (`paycheck-service.ts:493`, llamada `:500`). **ACTUALIZADO EN F08 (#113)**: hasta entonces `garnishments` no tenía un solo `INSERT` en `src/` —el criterio `payroll-output-tables-have-writers` (hoy `src/plan/criterios.ts:5021`, antes citado aquí como `:2405-2408`) lo medía en rojo y el catálogo lo decía en su fila (hoy `:1667`, antes citada como `:1664`)—, así que ningún camino de ejecución producía un importe distinto de cero. Hoy la puerta existe: `src/services/payroll/common/garnishment-service.ts` es el escritor y `mnemosine garnishment record|list|archive` la superficie. Lo que sigue abierto es todo lo de la lista de abajo.
...
- **Levy federal**: Pub 1494 depende de estado civil, dependientes y frecuencia; aquí es un importe manual (`:230`). Desde la migración 084 ese importe ya no puede faltar NI VALER CERO: un levy cuyo `metadata.exempt_amount` esté ausente, sea nulo, no parsee como número o sea cero no se guarda — las cuatro grafías acaban en el mismo `o.exempt_amount || 0`, que retiene el cien por ciento del ingreso disponible con `cap_applied` en null. (La primera redacción de la 084 miraba el TIPO del valor, así que paraba las dos primeras y admitía las dos últimas mientras esta línea decía que las paraba todas; hoy el CHECK exige que el valor parsee como número POSITIVO y el escritor niega el cero con su frase.) Lo que sigue sin existir es la TABLA de Pub 1494: la cifra se teclea.
- `max_withholding_pct`, `total_paid`, `total_owed` (`008:431, 438-440`) sin lector; `end_date` no decide nada (`:170` filtra sólo `is_active`; desde F08 la columna tiene dos lectores y los dos la IMPRIMEN): el embargo no se extingue al saldarse. F08 NO les puso escritor a propósito —una bandera para una columna que nadie lee publica un tope que no topa— y `garnishment archive` detiene la retención por `is_active`, que es lo único que el motor mira.
- Cuota administrativa del patrón por estado; remesa al acreedor (`payee_*`) sin puerta.
- **MX**: pensión alimenticia (LFT art. 110-V, 97-IV; CCF 308-323): el motor no corre para MX (`paycheck-service.ts:493`, antes citado aquí como `:287`, que es la rama de FIT/FICA; catálogo `:1677`, antes citado como `:1674`). F08 cierra la puerta en el escritor mientras no exista el parámetro: `garnishment record` rechaza el tipo `pension_alimenticia` y rechaza cualquier tipo contra un empleado cuyo `country_code` no sea `US`, porque los topes que el motor aplicaría son los de la CCPA —ley estadounidense quemada— y no la LFT en una tabla con vigencias. Y la 084 lo respeta en la BASE: su CHECK de los dos topes cubre `child_support` y NO `pension_alimenticia`, porque esos dos booleanos son entradas de la CCPA y exigírselos a una orden mexicana pararía `npm run migrate` en el despacho que tenga una en el expediente, con una sola salida — inventar dos respuestas de una ley que no la gobierna. Lo que sí se le exige es que, si los TRAE, sean booleanos: un `::boolean` sobre «yes» aborta la corrida de nómina entera. La mitad que depende del país vive en el censo de la migración, que sí puede unir `employees` y sí para por una `pension_alimenticia` de un empleado estadounidense.

En el mayor el importe **no se pierde**: va sumado en `post_tax_deductions` y de ahí a `garnishment_payable` (`gl-posting-service.ts:80`, `:147-150`). **CORRECCIÓN (F08)**: la cifra distinguible en `paychecks.garnishments` tampoco se pierde — la columna SÍ está en la lista del `INSERT` (`paycheck-service.ts:562`) y se alimenta de `breakdown.garnishments` (`:506`). Lo que decía esta línea, y lo que dice §1.21, era falso contra el árbol.
...
Por definir (**ambas**):
- No escribe `paycheck_taxes` (`008:309`; `grep "INSERT INTO paycheck_taxes"` vacío; sí lo lee `payroll.ts:481`; `doctor-service.ts:695` lo clasifica «numero-falso»; catálogo `:1592`), ni `employer_tax_liabilities` (§1.22), ni la columna `local_tax_withheld` (§1.12) de `paychecks`. **CORRECCIÓN (F08)**: `paychecks.garnishments` SÍ se escribe — está en la lista del `INSERT` (`paycheck-service.ts:562`) y su valor viene de `breakdown.garnishments` (`:506`); citarla aquí era un error de lectura que §1.19 repetía.
