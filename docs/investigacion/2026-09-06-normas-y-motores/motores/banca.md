...
- `grep -rnE "esContabilidadMexicana|pais-contable|incorporation_country|accounting_standard"` sobre `src/services/banking/` y `src/cli/bank-command.ts` devuelve **cero**. Ningún motor bancario consulta `src/services/jurisdiction/jurisdiction.ts`.
