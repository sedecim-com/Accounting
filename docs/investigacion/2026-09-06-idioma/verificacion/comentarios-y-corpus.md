...
| `src/services/jurisdiccion/jurisdiccion.ts` (encabezado de 33 líneas; umbral de cobertura «`:105`») | **no está en ningún commit**: es un archivo sin versionar de otra sesión | `git log --format=%h -- src/services/jurisdiccion/jurisdiccion.ts \| wc -l` → 0; `git status --short` → `?? src/services/jurisdiccion/` |
